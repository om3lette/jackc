...
void jackc_vm_code_bootstrap(vm_code_generator* generator) {
    ...
    if (jackc_strcmp(JACK_SP_REG, "sp") != 0) {
        VM_CODE_GEN_HELP_COMMENT_TAB(fd, "Initialize the stack pointer\n");
        jackc_fprintf(fd, "mv %s, sp\n", JACK_SP_REG);
    };

    if (generator->config->is_stack_growing_upwards) {
        VM_CODE_GEN_HELP_COMMENT_TAB(fd, "Allocate space for the upwards growing stack\n");
        jackc_fprintf(
            fd,
            "\tli %s, %d\n"
            "\tadd %s, %s, %s\n",
    ...
    VM_CODE_GEN_HELP_COMMENT_TAB(fd, "Prepare argc, argv for Main.main\n");
    vm_code_gen_stack_alloc(fd, 2, generator->config);
    jackc_fprintf(
        fd,
        "\tsw a0, 0(%s)\n"
        "\tsw a1, %d(%s)\n"
        "\tmv %s, %s\n",
        JACK_SP_REG,
        -config_offset_by_idx(generator->config, 1), JACK_SP_REG,
